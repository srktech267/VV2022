...
import org.firstinspires.ftc.teamcode.core.VVAutonQRReader;
...
@Autonomous(name="Auton_Blue" , group="AutonVV2022")

public class AutonComp4 extends VVAutonQRReader {
   ...
   public int robotParkingPosition  = 0 ;
    ...
    public void runOpMode() {
        ...
        telemetry.addData("robotParkingPosition", robotParkingPosition);
        telemetry.update();
        robotOps.stop();
        ...
        if (opModeIsActive()) {
            ...
            robotParkingPosition = getRobotParkingPosition();
            ...
            switch (robotParkingPosition)
            {
                case VVConstants.PARK_LEFT:
                {

                    robotOps.slideLeft(robot, .3, 2300);


                    robotOps.driveFront(robot, .3, 1800);

                    break;

                }
                case VVConstants.PARK_MIDDLE:
                {
                    robotOps.driveFront(robot, .4, 2000);
                    robotOps.driveBack(robot, .3, 1200);
                    break;
                }
                case VVConstants.PARK_RIGHT:
                {
                    robotOps.slideRight(robot, .3, 2400);
                    robotOps.driveFront(robot, .3, 2200);
                    break;
    ...
    public void loadCone()
    {
        ...
        robotOps.slideLeft(robot,.3,1075);
        ...
        clawClose();
        robot.armMotor.setPower(.2);
        robotOps.slideRight(robot,.3,1030);
