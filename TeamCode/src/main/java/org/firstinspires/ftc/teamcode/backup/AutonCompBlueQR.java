...
import org.firstinspires.ftc.teamcode.Auton.VVAutonQRReader;

@Autonomous(name="AutonBlue" , group="AutonVV2022")
@Disabled
public class AutonCompBlueQR extends VVAutonQRReader {
    ...
    public void runOpMode() {
        ...
        if (opModeIsActive()) {
            ...
            if (getRobotParkingPosition() > 0 ) {
                if (RobotParkingPosition == 1){

                    robotOps.driveFront(robot, .3, 3000);
                    robotOps.driveBack(robot, .3, 1500);
                    robotOps.slideRight(robot, .3, 2200);

                } else if (RobotParkingPosition == 2 ) {
                    robotOps.driveFront(robot, .3, 3000);
                    robotOps.driveBack(robot, .3, 1200);

                } else if (RobotParkingPosition == 3 ) {
                    robotOps.driveFront(robot, .3, 3000);
                    robotOps.driveBack(robot, .3, 1400);
